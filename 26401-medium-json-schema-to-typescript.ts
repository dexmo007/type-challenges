...
// ============= Your Code Here =============
type JSONNumber2TS<TEnumSchema> = TEnumSchema extends {
  enum: infer TEnum extends number[];
}
  ? TEnum[number]
  : number;

type JSONString2TS<TEnumSchema> = TEnumSchema extends {
  enum: infer TEnum extends string[];
}
  ? TEnum[number]
  : string;

type JSONObjectNotRequired<
  TKey,
  TRequired extends string[]
> = TRequired extends undefined
  ? never
  : TKey extends TRequired[number]
  ? never
  : TKey;

type JSONObjectIsRequired<
  TKey,
  T extends { required?: string[] },
  TTrue,
  TFalse
> = T extends {
  required: infer TRequired extends string[];
}
  ? TKey extends TRequired[number]
    ? TTrue
    : TFalse
  : TFalse;
type JSONObject<T> = { [key in keyof T]: T[key] };
type JSONObject2TS<TSchema> = TSchema extends {
  properties: infer TProperties;
  required?: infer TRequired extends string[];
}
  ? JSONObject<
      {
        [key in keyof TProperties as JSONObjectIsRequired<
          key,
          TSchema,
          key,
          never
        >]: JSONSchema2TS<TProperties[key]>;
      } & {
        [key in keyof TProperties as JSONObjectIsRequired<
          key,
          TSchema,
...
type JSONArray2TS<TSchema> = TSchema extends { items: infer TItemSchema }
  ? JSONSchema2TS<TItemSchema>[]
  : unknown[];

type JSONSchema2TS<TSchema> = TSchema extends {
  type: infer TType;
}
  ? TType extends "number"
    ? JSONNumber2TS<TSchema>
    : TType extends "boolean"
    ? boolean
    : TType extends "string"
    ? JSONString2TS<TSchema>
    : TType extends "object"
    ? JSONObject2TS<TSchema>
    : TType extends "array"
    ? JSONArray2TS<TSchema>
    : never
  : never;

type Test<T extends { properties: any; required?: string[] }> = {
  [key in keyof T["properties"] as T["required"] extends undefined
    ? key
    : never]: T["properties"][key];
};

type TestTest = Test<{ properties: { a: string; b: number } }>;

//   ^?

type Test22 = JSONObject<{ foo: string } & { bar?: string }>;
